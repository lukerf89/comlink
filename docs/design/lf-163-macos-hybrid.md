...
LF-163 design proposal, 2026-09-23. **Pending Luke's review.** This runnable native prototype implements an interaction proposal, not production dictation or approval to advance a phase.
...
| --- | --- | --- |
| Recording gesture | Toggle Start/Stop first; avoid accidental release ending long thoughts. | Luke approves toggle versus hold-to-talk. Hold-to-talk is deferred. |
| Dictation shortcut | Illustrative `⌥Space`; app-local in the preview. Menu bar is always available. | Configurable global binding, conflict detection, and an explicit enable step; do not assume the example key is available. |
...
After design review and reconciliation of the phase gates, ship **menu-bar manual dictation → pill → local processing → review → Copy**. Reuse the existing Rust core and schema v1. Defer global shortcuts, insertion, file/history/vocabulary GUI editing, and any system audio work. The palette may initially expose Start, mode selection, and review only; do not ship nonfunctional future commands.
...
| Phase 10 | Global hotkey/listen integration and explicit focused-field insertion need their own approved sub-missions and native tests. This issue does not authorize them. |
