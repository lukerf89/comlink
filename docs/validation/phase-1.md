...
- FFmpeg AVFoundation microphone capture, followed by the existing 16 kHz mono WAV normalization path.
- Minimum-duration/no-speech guard returning exit code `4`.
...
- A single whisper.cpp CPU retry (`-ng`) if the default GPU/Metal path fails.
...
- The first real whisper run exposed a Metal allocation failure; the adapter now retries once with `-ng`, and the real validation command passes.
...
Observed mocked stop-to-final latency: `138 ms`.
