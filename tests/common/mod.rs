...
pub struct MockOptions {
    ...
    pub silent: bool,
    /// The mock recorder starts a long-lived non-capture child (`sleep`,
    /// which ignores SIGINT as a background job) and writes its pid to
...
if [ -n "{descendant}" ]; then
  # A non-capture child (no chunk pattern) that, as a background job,
  # ignores SIGINT: only a later signal round stops it.
  sleep 300 &
  echo "$!" > "{descendant}"
fi
